...
(function () {
  ...
  /* --- menu hamburger (mobile): apre/chiude le voci del sito su tutte le pagine --- */
  (function () {
    const mnav = document.getElementById('mnav');
    const burger = document.querySelector('[data-navtoggle]');
    if (!mnav || !burger) return;
    document.addEventListener('click', e => {
      if (e.target.closest('[data-navtoggle]')) { const open = mnav.hidden; mnav.hidden = !open; burger.setAttribute('aria-expanded', String(open)); return; }
      if (!mnav.hidden && !e.target.closest('#mnav')) { mnav.hidden = true; burger.setAttribute('aria-expanded', 'false'); }
    });
  })();

  /* --- filtri a fisarmonica su mobile: ogni sezione (.sblock) si apre/chiude
     col titolo, cosi' non allungano la pagina. Sopra i 900px restano aperti. --- */
  (function () {
    const mq = matchMedia('(max-width:900px)');
    const blocks = [].slice.call(document.querySelectorAll('.side .sblock'));
    if (!blocks.length) return;
    function apply() {
      blocks.forEach(b => {
        if (mq.matches) { b.classList.add('acc'); if (!b.classList.contains('acc-open')) b.classList.add('acc-closed'); }
        else { b.classList.remove('acc', 'acc-closed'); }
      });
    }
    apply();
    document.addEventListener('click', e => {
      const h = e.target.closest('.side .sblock.acc h3'); if (!h) return;
      const b = h.closest('.sblock'); b.classList.toggle('acc-closed'); b.classList.toggle('acc-open', !b.classList.contains('acc-closed'));
    });
